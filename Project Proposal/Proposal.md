...
[Measures of Success](https://github.com/Hinrichsta/FA23-Cyber8420/blob/main/Project%20Proposal/Proposal.md#measures-of-success)
...
[Security issues](https://github.com/Hinrichsta/FA23-Cyber8420/blob/main/Project%20Proposal/Proposal.md#security-issues)  
[Perceived Threats](https://github.com/Hinrichsta/FA23-Cyber8420/blob/main/Project%20Proposal/Proposal.md#perceived-threats)
...
[Contributor Agreement](https://github.com/Hinrichsta/FA23-Cyber8420/blob/main/Project%20Proposal/Proposal.md#Contributor-Agreement)
...
GetFINHelp (GFH) Bank strategic plan compliments the three pillars of data integrity: Confidentiality, Integrity and Availability (CIA).  GFH Bank focuses on the latest advanced technological advancements in computing while maintaining the integrity of the data from both internal and external data leakage.  

For example, our software developers are certified and trained in software assurance. One of the key elements of software assurance, design and development is to understand key fundamentals throughout the Software Development Lifecycle (SDLC). 

## Secure Software Development Framework (SSDF)  
The Secure Software Development Framework (SSDF) version 1.1 instituted by the National Institute of Standards Technology (NIST) SP 800-218 is a process which can be implemented for mitigating the risk of software vulnerabilities. 
...
Cryptography is a technique used to mask data from being read or accessible through malicious actors. There are several solutions cryptography can be implemented to encrypt data. Refer to [Cryptography | NIST](https://www.nist.gov/cryptography) for further guidance.  
...
Security Objectives:
* Limit access control to system files and folders
* Authenticate system access by user name, password, token, etc.,
* Enforce security policies across the network enterprise
* Limit/deny access the network by enforcing a password compliance policy
...
There was no encryption in protecting consumers financial data. All data files must use encryption features when transporting files over an open or closed network.   
...
## GFH Environment
GetFINHelp (GFH) Bank uses a Nextcloud Server to share secure documents with customers and clients. Customers and Clients can also upload securely to the system for File sharing to prevent PII (Personally Identifiable Information) data spillage. Nextcloud Server shares can be accessed by only authorized employees of GFH Bank and GFH clients. Example of shared files  include Tax Forms, Bank Statements, Invoices, ACH Payment Receipts, etc. 

Nextcloud Files will have moderate interactions with Banking software to identify user accounts to file shares. This process will ensure user folders are only accessible from authorized account holders within the GFH Bank internal network.  

Files will be accessible by customers and clients from the Web Portal and Mobile App with the ability to download and upload. Based on links or access to files from Web and Mobile App some interaction will exist there for direct access to the appropriate shares. Security will need to be involved in data at rest as well as in transit for uploads and downloads. User authentication of Nextcloud Server will require Multi-Factor Authentication for access and will have periodic timeouts to ensure inactive sessions are closed. GFH Bank will follow guidance  based upon industry best practices such as password and security policies, encryption standards. 

GFH Bank is a financial institution which is regulated and is in compliance with Payment Card Industry (PCI) Data Security Standard (DSS) governing body.  

PCI and DSS standards according to federal, state and local laws. GFH Bank is a financial institution which is considered the gold standard in using best practices in maintaining data security. GFH Bank also follows the industry best practices with the Software Development Lifecycle (SDLC) framework to ensure processes are following in software assurance and development.  


## System Engineering View
![image](https://github.com/Hinrichsta/FA23-Cyber8420/assets/143133739/af9213bd-ff77-4c56-be89-d61244f0efb5)
...
## Project Description
The contributors consist of a growing number of highly reviewed individuals. As of today’s document, writing, the number has grown to 890.  

As the community grows the support and contributions grow with it making it an ideal choice for open-source software. Activity for this open-source software has been steady since the software was first created as a repository and is updated through contributions and branches monthly. The community of followers is active and helpful for resolving potential issues.  
